 ...
 * D1 Database Helper
 *
 * Mail core (messages, contacts, tags, settings) lives here.
 * Tagging + auto-response rules live in db-rules.js, calendar events in db-calendar.js.
 * They're merged into the exported `DB` object at the bottom so call sites stay unchanged.
 */
import { RulesDB } from './db-rules.js';
import { CalendarDB } from './db-calendar.js';
...
export const DB = {
  ...
  // Spread in domain modules. Each is self-contained (no shared helpers),
  // so the merge is purely organisational and call sites remain unchanged.
  ...RulesDB,
  ...CalendarDB
};
